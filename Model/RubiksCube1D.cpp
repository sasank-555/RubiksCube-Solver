...
class RubiksCube1D : public RubiksCube {
    ...
    int getIndex(int face, int row, int col) const {
        ...
        void rotateFace(int ind){
            ...
            for(int i = 0; i < 3; i++){
                for(int j = 0; j < 3; j++){
                    temp[i][j] = cube[getIndex(ind, i, j)];
                }
            }   
            for(int i = 0; i < 3; i++) cube[getIndex(ind, 0, i)] = temp[2-i][0];
            for(int i = 0; i < 3; i++) cube[getIndex(ind, i, 2)] = temp[0][i];
            for(int i = 0; i < 3; i++) cube[getIndex(ind, 2, 2 - i)] = temp[i][2];
            for(int i = 0; i < 3; i++) cube[getIndex(ind, 2- i, 0)] = temp[2][2 - i];
        }
    public:
        char cube[54] = {};
        RubiksCube1D(){
    ...
    bool operator==(const RubiksCube1D &other) const {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                for (int k = 0; k < 3; k++) {
                    if (this->cube[getIndex(i,j,k)] != other.cube[getIndex(i,j,k)]) return false;
                }
            }
        }
        return true;
    }
    RubiksCube1D &operator=(const RubiksCube1D &other) {
        if (this != &other) {
            for (int i = 0; i < 6; i++) {
                for (int j = 0; j < 3; j++) {
                    for (int k = 0; k < 3; k++) {
                        this->cube[getIndex(i,j,k)] = other.cube[getIndex(i,j,k)];
                    }
                }
            }
        }
